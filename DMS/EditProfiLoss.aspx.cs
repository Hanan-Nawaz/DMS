...
namespace DMS
{
    public partial class EditProfiLoss : System.Web.UI.Page
    {
        ...
        void getDatafromDB()
        {
            ...
            while (reader.Read())
            {
                ...
                string payr = reader["PayrollTax"].ToString();
                string PercentagePayroll = Convert.ToString(payr.Replace(CultureInfo.InvariantCulture.NumberFormat.PercentSymbol, ""));
                tbPayrolExpense.Text = PercentagePayroll;

                string card = reader["CardFee"].ToString();
                string cardf = Convert.ToString(card.Replace(CultureInfo.InvariantCulture.NumberFormat.PercentSymbol, ""));
                tbCardFee.Text = cardf;

                string ins = reader["Insurance"].ToString();
                string insf = Convert.ToString(ins.Replace(CultureInfo.InvariantCulture.NumberFormat.PercentSymbol, ""));
                tbInsuranceExp.Text = insf;
        ...
        public void SavetoDB()
        {
            ...
            string sqlStmt = "UPDATE ProfitLoss SET Id = @Id,Model = @Model,GrossSales = @GrossSales,CashSales = @CashSales,CashSalesPercentage = @CashSalesPercentage,NonCashSales = @NonCashSales,NonCashSalesPercentage = @NonCashSalesPercentage,TotalSales = @TotalSales,TotalSalesPercentage = @TotalSalesPercentage,Date = @Date,PayrollTax = @PayrollTax,Insurance = @Insurance,CardFee = @CardFee,Operator = @Operator,TotalCost = @TotalCost,TotalCostPercentage = @TotalCostPercentage,TotalProfit = @TotalProfit,TotalProfitPercentage = @TotalProfitPercentage,LabourModel = @LabourModel,CostofLabor = @CostofLabor,LabourPercentage = @LabourPercentage,TotalLabor = @TotalLabor,SetInsurance = @SetInsurance,SetInsurancePercentage = @SetInsurancePercentage,GetInsurance = @GetInsurance,SetPayrollTax = @SetPayrollTax,PayrollTaxPercentage = @PayrollTaxPercentage,GetPayroll = @GetPayroll,OpEx = @OpEx,OpExPercentage = @OpExPercentage,SetCardFee = @SetCardFee,CardProcessingFee = @CardProcessingFee,CardProcessingFeePercentage = @CardProcessingFeePercentage,TotalOpEx = @TotalOpEx,TotalOpExPercentage = @TotalOpExPercentage,Managed = @Managed,ManagedPercentage = @ManagedPercentage,Owner = @Owner,OwnerPercentage = @OwnerPercentage,Notes = @Notes,CurrentDate = @CurrentDate WHERE Id = '" + tb_PL_id.Text + "'";
            ...
            try
            {
                ...
                if (i > 0)
                {
                    ClientScript.RegisterClientScriptBlock(this.GetType(), "k", "swal('congratulations', 'Profit Loss Data Updated to DB Succesfully :)', 'success')", true);
                }
                else
                {
